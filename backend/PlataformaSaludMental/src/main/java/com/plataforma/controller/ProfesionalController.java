...
public class ProfesionalController {
	...
	public ResponseEntity<?> registrarProfesional(@RequestBody @Valid RegistroProfesionalDTO registrarProfesional,
		...
		if (profesionalService.obtenerProfesionalPorDni(registrarProfesional.getDni()) != null) {
			return ResponseEntity.badRequest().body(Map.of("error", "Ya existe una cuenta registrada con este DNI."));
		}
		...
		try {
			...
			return ResponseEntity.status(500)
					.body(Map.of("error", "No se pudo completar el registro, intente nuevamente más tarde."));
		}
	}
	...
	public ResponseEntity<?> loginProfesional(@RequestBody LoginProfesional loginProfesional) {
		...
		if (profesional == null) {
			return ResponseEntity.badRequest().body(Map.of("error", "DNI incorrecto."));
		}
	...
	public ResponseEntity<?> subirRequisitos(@PathVariable Long id, @RequestParam String universidad,
			@RequestParam String matricula, @RequestParam String titulo, @RequestParam String categoria,
		...
		try {
			...
			return ResponseEntity
					.ok(Map.of("mensaje", "Documentos recibidos. Su perfil está pendiente de validación."));
	...
	public ResponseEntity<?> actualizarPerfil(@PathVariable Long id, @RequestParam("descripcion") String descripcion,
		...
		try {
			...
			if (descripcion.length() > 500)
				return ResponseEntity.badRequest()
						.body(Map.of("error", "La descripción no puede superar los 500 caracteres."));
			...
			return ResponseEntity.internalServerError()
					.body(Map.of("error", "Error al procesar la solicitud. Intente nuevamente más tarde."));
		}

		return ResponseEntity.ok(Map.of("message", "Actualización exitosa."));
	}
}
